...
export const metadata: Metadata = {
  title: "Sobre o Grupo RBM | Ecossistema Empresarial Integrado",
  description: "Conheça a história, missão, visão e valores do Grupo RBM. Unimos contabilidade, marketing, desenvolvimento de software e mídia digital.",
};
  ...
  return (
    <div className="bg-white py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumb items={[{ label: "Sobre o Grupo RBM" }]} />

        {/* Hero Section Page */}
        <div className="bg-[#061F40] text-white rounded-3xl p-8 sm:p-12 mb-16 relative overflow-hidden">
          <div className="max-w-3xl space-y-4 relative z-10">
            <span className="text-xs font-bold uppercase tracking-wider text-rbm-lime">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {aboutStats.map((stat) => (
            <div key={stat.id} className="bg-[#F5F7FA] p-6 rounded-2xl border border-rbm-border text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
          ...
          <div className="lg:col-span-6 bg-[#F5F7FA] p-8 rounded-3xl border border-rbm-border space-y-6">
        ...
        {/* Values */}
        <div className="mb-16">
          <h2 className="text-2xl font-extrabold font-heading text-rbm-navy mb-8 text-center">
            Nossos Valores Fundamentais
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {aboutContent.values.map((val, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl border border-rbm-border shadow-sm flex items-start space-x-3">
                <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
                <span className="text-xs font-semibold text-rbm-dark">{val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Companies Ecosystem */}
        <div className="bg-[#F5F7FA] rounded-3xl p-8 sm:p-12 mb-16 border border-rbm-border">
          <h2 className="text-2xl font-extrabold font-heading text-rbm-navy mb-6">
          ...
          <p className="text-xs sm:text-sm text-gray-700 leading-relaxed mb-8 max-w-3xl">
            Cada uma das cinco empresas do Grupo RBM atua de forma independente ou em perfeita sinergia. Isso significa que sua empresa pode começar com um serviço específico e expandir a integração conforme novas demandas surgirem.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {companies.map((c) => (
              <div key={c.id} className="bg-white p-5 rounded-2xl border border-rbm-border space-y-3">
        ...
        <div className="text-center bg-[#061F40] text-white p-10 rounded-3xl space-y-4">
          ...
          <p className="text-xs sm:text-sm text-gray-300 max-w-xl mx-auto">
            Fale com a nossa equipe de especialistas e descubra como o atendimento integrado do Grupo RBM pode gerar resultados reais.
          </p>
